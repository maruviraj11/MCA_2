...
public class ReportServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String filename = "library_" + type + "_report_" + timestamp + ".xls";
    ...
    private void writeIssued(PrintWriter out, boolean overdueOnly) {
        ...
        out.println("<tr><th>Request ID</th><th>User Email</th><th>Book Title</th><th>Issue Date</th><th>Due Date</th></tr>");
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(query);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                out.println("<tr><td>" + htmlCell(String.valueOf(rs.getInt("request_id"))) + "</td>" +
                        "<td>" + htmlCell(rs.getString("user_email")) + "</td>" +
                        "<td>" + htmlCell(rs.getString("book_title")) + "</td>" +
                        "<td>" + htmlCell(fmtTs(rs.getTimestamp("issue_date"))) + "</td>" +
                        "<td>" + htmlCell(fmtDate(rs.getDate("due_date"))) + "</td></tr>");
            }
        } catch (SQLException e) {
            out.println("<tr><td colspan=\"5\">" + htmlCell("Failed to generate report: " + e.getMessage()) + "</td></tr>");
        }
    ...
    private void writeFines(PrintWriter out) {
        ...
        out.println("<html><head><meta charset=\"UTF-8\"></head><body>");
        out.println("<table border=\"1\" cellspacing=\"0\" cellpadding=\"4\">");
        out.println("<tr><th>Request ID</th><th>User Email</th><th>Book Title</th><th>Return Date</th><th>Fine Amount</th><th>Paid Amount</th><th>Due Amount</th><th>Fine Paid</th><th>Fine Paid Date</th></tr>");
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(query);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ...
                out.println("<tr><td>" + htmlCell(String.valueOf(rs.getInt("request_id"))) + "</td>" +
                        "<td>" + htmlCell(rs.getString("user_email")) + "</td>" +
                        "<td>" + htmlCell(rs.getString("book_title")) + "</td>" +
                        "<td>" + htmlCell(fmtTs(rs.getTimestamp("return_date"))) + "</td>" +
                        "<td>" + htmlCell(fine.toPlainString()) + "</td>" +
                        "<td>" + htmlCell(paid.toPlainString()) + "</td>" +
                        "<td>" + htmlCell(due.toPlainString()) + "</td>" +
                        "<td>" + htmlCell(rs.getInt("fine_paid") == 1 ? "yes" : "no") + "</td>" +
                        "<td>" + htmlCell(fmtTs(rs.getTimestamp("fine_paid_date"))) + "</td></tr>");
            }
        } catch (SQLException e) {
            out.println("<tr><td colspan=\"9\">" + htmlCell("Failed to generate report: " + e.getMessage()) + "</td></tr>");
        }
        out.println("</table></body></html>");
    }
    ...
    private String htmlCell(String value) {
