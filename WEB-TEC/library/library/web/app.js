(function () {
    ...
    function validateAddBook(form) {
        ...
        var quantity = getInput(form, "quantity");
        ...
        if (quantity) {
            attachClearOnInput(quantity);
            var qv = trim(quantity.value);
            if (isEmpty(qv)) setValidity(quantity, "Quantity is required.");
            else if (!isPositiveIntegerString(qv)) setValidity(quantity, "Quantity must be an integer greater than or equal to 1.");
            else setValidity(quantity, "");
        }
